...
export interface LayoutProps {
  ...
  noWrap?: boolean;
  /**
   * ТЕКСТ ОБРЕЗАН ПО ШИРИНЕ КОРОБКИ И ЗАКОНЧЕН МНОГОТОЧИЕМ.
   *
   * Это ТРОЙКА CSS-свойств, а не догадка: `white-space: nowrap` +
   * `overflow: hidden` + `text-overflow: ellipsis`. Тем и ценно, что
   * выразимо в экспорте один в один — в отличие от `wrapInset`, у
   * которого прямого CSS-эквивалента нет и который приходится
   * пересчитывать в ширину переноса.
   *
   * Зачем в модели отдельный флаг, если есть `noWrap`. `noWrap` говорит
   * только «перенос не делался», и коробка при этом растёт по тексту:
   * длинное сообщение коммита в таблице файлов GitHub разворачивалось на
   * 474px в колонке шириной 389 и наезжало на дату. Обрезка — это ещё и
   * ПОТОЛОК: строка не выходит за коробку, а хвост заменяется многоточием,
   * ровно как её показал браузер.
   *
   * Ставится по ИЗМЕРЕННОМУ факту из снимка (`SnapNode.tr`): обрезка и
   * объявлена, и случилась. Объявление без переполнения в модель не идёт.
   *
   * Отсутствие свойства = прежнее поведение: старые сохранения читаются.
   */
  ellipsis?: boolean;

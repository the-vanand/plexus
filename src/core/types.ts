...
export interface LayoutProps {
  ...
  noWrap?: boolean;
  /**
   * СУЖЕНИЕ СТРОК ОБТЕКАНИЕМ (`float` рядом), в пикселях.
   *
   * По спецификации плавающий блок выведен из потока, и соседний блок
   * НЕ становится уже: его коробка остаётся полной ширины, а укорачиваются
   * только СТРОКИ внутри неё. Модель холста этого не выражала вовсе, и
   * абзац шириной 912px рядом с инфобоксом Википедии (заход 331px справа)
   * укладывался в четыре строки вместо шести: высота 104px вместо 156,
   * всё нижележащее уезжало вверх и лезло под карточку — 23 наложения на
   * одной статье.
   *
   * Выразить это шириной коробки нельзя: ширина у абзаца измеренная и
   * верная, менять её значило бы врать про `dw` и ломать вложенное
   * содержимое. Поэтому вводится ровно то, чем обтекание и является:
   * ширина коробки одна, ширина ПЕРЕНОСА — меньше на этот просвет.
   *
   * Величина измеренная, а не гадательная: горизонтальный заход плавающего
   * блока на коробку соседа, взвешенный долей высоты, на которой они
   * пересекаются (сколько строк идёт рядом с флоатом — ровно эта доля).
   *
   * Отсутствие поля = прежнее поведение: старые сохранения читаются.
   */
  wrapInset?: number;

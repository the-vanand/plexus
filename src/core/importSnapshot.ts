...
function importSnapshotToDocInner(
  ...
  const walk = (
    ...
    /* ПОЛНОСТЬЮ ПРОЗРАЧНОЕ ПОДДЕРЕВО НА СТРАНИЦЕ НЕ СУЩЕСТВУЕТ.
       `opacity: 0` — групповой эффект: браузер гасит узел ВМЕСТЕ со всеми
       детьми, а `getComputedStyle` у детей при этом честно возвращает их
       собственную единицу. Импорт переносил прозрачность только на сам
       узел, и дети рисовались непрозрачными: у каждой кнопки YouTube
       внутри лежит чёрная подложка нажатия (`ytSpecTouchFeedbackShape`,
       чёрный фон, скругление, opacity 0) — на холсте она проступала
       чёрной пилюлей поверх кнопки, и таких узлов на yt-watch 384.
       Это тот же класс, что `visibility: hidden`, который сборщик
       отсекает ещё при съёмке; opacity сборщик записывает, поэтому
       отсекаем здесь. Порог строгий (=== 0): полупрозрачное содержимое
       настоящее, его не трогаем. */
    if (parseFloat(n.s["opacity"] ?? "1") === 0) {
      collapsed += 1;
      return;
    }

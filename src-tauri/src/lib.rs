...
async fn capture_snapshot(
    ...
    // Адрес разбираем ДО открытия окна и пускаем только http и https:
    // capability окна снимка выдана этим двум схемам, а file или иная
    // схема в окне снимка нам не нужны вовсе.
    //
    // Комментарий здесь СТРОЧНЫЙ намеренно. Блочные комментарии в Rust
    // вложенные, и в записи схемы со звёздочкой компилятор видит открытие
    // вложенного комментария. Ровно на этом сборка падала с E0758: внешний
    // комментарий оставался незакрытым и съедал полторы сотни строк.
